...
この構成にしておくと
  ...
  ・broker.py が root.usda を読めば合成後（＝編集後）の状態が得られる
という往復が成立します。

Isaac Sim の Window > Script Editor に貼り付けて、パス 2 行を書き換えて実行。
このスクリプトは Isaac Sim 側で完結し、broker とは通信しません
（broker は保存された root.usda をファイルとして読むだけです）。
"""

BASE_USD = r"C:/usd_bridge/base.usdc"   # Blender からエクスポートしたファイル
ROOT_USD = r"C:/usd_bridge/root.usda"   # これから作る編集用ファイル

import os
import omni.usd
from pxr import Usd, UsdGeom, Sdf

# --- 入力チェック（パス間違いは空の stage になって気づきにくいので先に弾く）---
if not os.path.exists(BASE_USD):
    raise FileNotFoundError(
        f"base.usdc が見つかりません: {BASE_USD}\n"
        "  Blender の File > Export > Universal Scene Description で\n"
        "  書き出したファイルのパスを指定してください。")
if os.path.abspath(BASE_USD) == os.path.abspath(ROOT_USD):
    raise ValueError("BASE_USD と ROOT_USD が同じファイルです。別のパスにしてください。")
...
print()
print("次のステップ: 別のコンソールで broker を起動してください")
print(f"    .venv/bin/python broker.py --stage \"{ROOT_USD}\"")
print("  (Windows: .venv\\Scripts\\python.exe broker.py --stage \"%s\")" % ROOT_USD)
