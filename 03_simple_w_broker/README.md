...
```python
BASE_USD = r"C:/usd_bridge/base.usdc"
ROOT_USD = r"C:/usd_bridge/root.usda"
```
...
# Windows
.venv\Scripts\python.exe broker.py --stage C:\usd_bridge\root.usda
```
...
Blender 側の常駐は、**別の .blend を開いても継続します**（Blender プロセスが生きている限り有効）。ただし `BASE_USD` の出力先は変わらないので、別プロジェクトの .blend を開いたまま保存すると、そちらの内容が `base.usdc` を上書きします。プロジェクトを切り替えるときは一度 `MODE = "stop"` で解除してください。
